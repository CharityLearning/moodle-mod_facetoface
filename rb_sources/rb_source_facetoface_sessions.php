...
class rb_source_facetoface_sessions extends rb_base_source {
    ...
    function __construct() {
        ...
        $this->sourcetitle = get_string('sourcetitle', 'report_reportbuilder');
        parent::__construct();
    }
    ...
    protected function define_columnoptions() {
        $columnoptions = array(
            new rb_column_option(
                'session',              // type
                'capacity',             // value
                get_string('sesscapacity', 'report_reportbuilder'),     // name
            ...
            new rb_column_option(
                'session',
                'numattendees',
                get_string('numattendees', 'report_reportbuilder'),
                'attendees.number',
            ...
            new rb_column_option(
                'session',
                'details',
                get_string('sessdetails', 'report_reportbuilder'),
                'sessions.details',
                array(
                    'joins' => 'sessions',
            ...
            new rb_column_option(
                'session',
                'duration',
                get_string('sessduration', 'report_reportbuilder'),
            ...
            new rb_column_option(
                'status',
                'statuscode',
                get_string('status', 'report_reportbuilder'),
                'status.statuscode',
                array(
                    'joins' => 'status',
             ...
             new rb_column_option(
                'session',
                'discountcode',
                get_string('discountcode', 'report_reportbuilder'),
                'base.discountcode'
            ),
            new rb_column_option(
                'session',
                'normalcost',
                get_string('normalcost', 'report_reportbuilder'),
                'sessions.normalcost',
                array(
                    'joins' => 'sessions',
                )
            ...
            new rb_column_option(
                'session',
                'discountcost',
                get_string('discountcost', 'report_reportbuilder'),
            ...
            new rb_column_option(
                'facetoface',
                'name',
                get_string('ftfname', 'report_reportbuilder'),
                'facetoface.name',
            ...
            new rb_column_option(
                'facetoface',
                'namelink',
                get_string('ftfnamelink', 'report_reportbuilder'),
                "facetoface.name",
                array(
                    'joins' => array('facetoface','sessions'),
                    'displayfunc' => 'link_f2f',
                    'defaultheading' => get_string('ftfname', 'report_reportbuilder'),
            ...
            new rb_column_option(
                'date',
                'sessiondate',
                get_string('sessdate', 'report_reportbuilder'),
                'sessiondate.timestart',
                array(
                    'extrafields' => null /*array('timezone' => 'sessiondate.sessiontimezone')*/,
                    'joins' =>'sessiondate',
            ...
            new rb_column_option(
                'date',
                'sessiondate_link',
                get_string('sessdatelink', 'report_reportbuilder'),
                'sessiondate.timestart',
                array(
                    'joins' => 'sessiondate',
                    'displayfunc' => 'link_f2f_session',
                    'defaultheading' => get_string('sessdate', 'report_reportbuilder'),
                    'extrafields' => null /*array('session_id' => 'base.sessionid', 'timezone' => 'sessiondate.sessiontimezone')*/
                )
            ),
            new rb_column_option(
                'date',
                'datefinish',
                get_string('sessdatefinish', 'report_reportbuilder'),
            ...
            new rb_column_option(
                'date',
                'timestart',
                get_string('sessstart', 'report_reportbuilder'),
                'sessiondate.timestart',
                array('joins' => 'sessiondate', 'displayfunc' => 'nice_datetime')
                //array('extrafields' => array('timezone' => 'sessiondate.sessiontimezone'), 'joins' => 'sessiondate', 'displayfunc' => 'nice_time_in_timezone')
            ),
            new rb_column_option(
                'date',
                'timefinish',
                get_string('sessfinish', 'report_reportbuilder'),
                'sessiondate.timefinish',
                array('joins' => 'sessiondate', 'displayfunc' => 'nice_datetime')
                //array('extrafields' => array('timezone' => 'sessiondate.sessiontimezone'), 'joins' => 'sessiondate', 'displayfunc' => 'nice_time_in_timezone')
            ),
            new rb_column_option(
                'session',
                'cancellationdate',
                get_string('cancellationdate', 'report_reportbuilder'),
            ...
            new rb_column_option(
                'session',
                'cancellationreason',
                get_string('cancellationreason', 'report_reportbuilder'),
            ...
            /*new rb_column_option(
                'room',
                'name',
                get_string('roomname', 'report_reportbuilder'),
                'room.name',
                array('joins' => 'room')
            ),
            new rb_column_option(
                'room',
                'building',
                get_string('building', 'report_reportbuilder'),
                'room.building',
                array('joins' => 'room')
            ),
            new rb_column_option(
                'room',
                'address',
                get_string('address', 'report_reportbuilder'),
                'room.address',
                array('joins' => 'room')
            ),
            new rb_column_option(
                'room',
                'capacity',
                get_string('roomcapacity', 'report_reportbuilder'),
                'room.capacity',
                array('joins' => 'room')
            ),
            new rb_column_option(
                'room',
                'description',
                get_string('roomdescription', 'report_reportbuilder'),
                'room.description',
                array('joins' => 'room')
            ),*/
        );
    ...
    protected function define_filteroptions() {
        $filteroptions = array(
            ...
            new rb_filter_option(
                'session',
                'capacity',
                get_string('sesscapacity', 'report_reportbuilder'),
                'number'
            ),
            new rb_filter_option(
                'session',
        ...
        if ($session_fields = $DB->get_records('facetoface_session_field', null, '', 'id,name')) {
            foreach ($session_fields as $session_field) {
                ...
                $columnoptions[] = new rb_column_option(
                    'session',
                    $key,
                    get_string('sessionx', 'report_reportbuilder', $name),
                    $key.'.data',
                    array('joins' => $key)
                );
            }
    ...
    protected function add_facetoface_session_custom_fields_to_filters(&$filteroptions) {
        ...
        $possible_filters = array(
            /*'pilot' => array(
                'text' => get_string('pilot', 'report_reportbuilder'),
                'type' => 'select',
                'options' => array('selectchoices' => array('Yes' => get_string('yes'), 'No' => get_string('no')))
            ),
            'audit' => array(
                'text' => get_string('audit', 'report_reportbuilder'),
                'type' => 'select',
                'options' => array('selectchoices' => array('Yes' => get_string('yes'), 'No' => get_string('no')))
            ),
            'coursedelivery' => array(
                'text' => get_string('coursedelivery', 'report_reportbuilder'),
                'type' => 'select',
                'options' => array(
